...
const errorMappings: Record<string, string> = {
  ...
  'email.emailAlreadyExists': 'inputs.email.validation.server.emailAlreadyExists',
  'email.invalid': 'inputs.email.validation.invalid',
  'email.required': 'inputs.email.validation.required',
  'password.incorrectPassword': 'inputs.password.validation.server.incorrectPassword',
...
export function getErrorTranslationKey(error: ApiError): string {
  for (const [field, errorCode] of Object.entries(error)) {
    const translationKey = errorMappings[`${field}.${errorCode}`] || errorMappings[errorCode] || errorMappings[field];
...
export function getFieldErrorTranslationKey(error: ApiError, field: string): string | undefined {
