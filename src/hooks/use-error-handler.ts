...
import { getErrorTranslationKey, getFieldErrorTranslationKey } from '@/services/helpers/error-mapper';
...
export default function useErrorHandler() {
  ...
  const translateError = useCallback((translationKey: string) => {
    if (translationKey.startsWith('inputs.')) {
      return tSignIn(translationKey) || tSignUp(translationKey) || tForgotPassword(translationKey) || tCommon(translationKey);
  ...
  const handleFieldError = useCallback((error: any, field: string) => {
    if (error && typeof error === 'object') {
